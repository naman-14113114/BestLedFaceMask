...
export type ProductPriceKey = "buudy" | "currentbody" | "omnilux" | "shark" | "drdenis";
...
export const advertorialMarkets: Record<AdvertorialMarketKey, AdvertorialMarket> = {
  uk: {
    key: "uk",
    ...
    currencyCode: "GBP",
    buudyUrl: "https://uk.buudy.com/products/buudy-led-mask",
    productPrices: {
      buudy: {
...
export const advertorialMarketRoutes = Object.values(advertorialMarkets).map((market) => market.route);
...
export function getAdvertorialMarketByRoute(route: string) {
  return Object.values(advertorialMarkets).find((market) => market.route === route) ?? advertorialMarkets.uk;
}
